...
import {toast} from "react-toastify";
 const Card = (props) => {
    ...
    let setLikesCourses=props.setLikesCourses;
    function clickHandler(){
        if(likedCourses.includes(course.id)){
    //pehle se liked h
        setLikesCourses((prev) => prev.filter((cid) => (cid !== course.id)));
        toast.warning("liked removed");
        }
        else{
        //pehle se liked nhi h
        if(likedCourses.length === 0){
            setLikesCourses([course.id]);
        }
        else{
            setLikesCourses((prev) => [...prev, course.id]);
        }
